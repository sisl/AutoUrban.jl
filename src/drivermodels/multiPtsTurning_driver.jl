type MultiPtsTurningDriver <: DriverModel{AccelSteeringDirection}
    Δt::Float64
    siji::AccSteerDriver
...
function excuteAction!(model::MultiPtsTurningDriver,acc::Float64,direction::Int,scene::Union{Scene,Frame{Entity{VehicleState, BicycleModel, Int}}}, roadway::Roadway, ego_index::Int)
    veh=scene[ego_index]
...
function setPts!(model::MultiPtsTurningDriver,PtsIn::Matrix{Float64})
    model.Pts=PtsIn
    model.index=0
    model
end
...
function getAccSteer!(model::MultiPtsTurningDriver,scene::Union{Scene,Frame{Entity{VehicleState, BicycleModel, Int}}}, roadway::Roadway, ego_index::Int)
    if model.index==6 || model.index>model.N
    #if model.index>model.N
        #println("index reach ", model.index)
        setLaneChangingPts_with_direction!(model, scene, roadway, ego_index,model.turning_direction)
    end
    if model.index==0
        #println("index is 0!")
        setLaneChangingPts_with_direction!(model, scene, roadway, ego_index,model.turning_direction)
        ...
        if model.subindex==0
            #compute new command
            getCommands!(model,scene, roadway,ego_index)
