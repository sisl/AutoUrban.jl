...
function connect_two_points_by_circle(
    A::VecSE2{T}, B::VecSE2{T};s_base::T = zero(T)
) where T <: Real
    ...
    for j in 1:ncurvepts_per_turn
        ...
        curvepts[j] = CurvePt(
            VecSE2(
                C + polar(r, lerp(posangle_a, posangle_b, t)),
                lerp(attangle_a, attangle_b, t)),
            s, k, kd)
    end
    ...
    if abs_diff < ANG_THD
        ...
        if abs(abs_diff - pi) < ANG_THD
            ...
            if pojection_length > ANG_THD
                ...
                circlepts, total_length = connect_two_points_by_circle(
                    P, B;s_base = pojection_length)
            ...
            elseif pojection_length < -ANG_THD
                ...
                push!(curvepts, CurvePt(
                    B, total_length + abs(pojection_length), 0.0, 0.0))
            else # no need of P
                circlepts, total_length = connect_two_points_by_circle(
                    A, B;s_base = 0.0)
                curvepts = vcat(curvepts, circlepts)
            ...
            if projection_length >= 0 && projection_lengthB >= 0
                ...
                push!(curvepts, CurvePt(
                    P, projection_length + projection_lengthB, 0.0, 0.0))
                circlepts, total_length = connect_two_points_by_circle(
                    P, B;s_base = projection_length + projection_lengthB)
                curvepts = vcat(curvepts, circlepts)
                push!(curvepts, CurvePt(B, total_length, 0.0, 0.0))
...
function connect_two_lane_general!(
    source::Lane,
    dest::Lane,
    seg_id::Int,
    lane_width::Float64;
    ...
    seg12.lanes[1] = Lane(
        tag12, curvepts, width = lane_width,
...
function connect_two_seg!(
    source::RoadSegment{T},
    dest::RoadSegment{T},
    roadway::Roadway{T};
    ...
    for i = 1:length(curveptsss[1])
        ...
        for j =  1:length(connections)
            ...
            seg12.lanes[j] = Lane(
                tag12, curveptsss[j][i],
                width = source.lanes[connections[j][1]].width,
...
function connect_two_seg_general!(
    source::RoadSegment{T},
    dest::RoadSegment{T},
    roadway::Roadway{T};
...
function gen_connected_lanes(
    ;nlanes::Int = 1,
    roadlength::Float64 = 5.0,
...
function gen_intersection(
    ...
    seg2 = gen_straight_segment(
        ...
        roadlength,
        origin = VecSE2(
            origin.x + roadlength + 0.5*lane_width, origin.y - 0.5*lane_width, -pi/2),
        lane_widths = lane_widths,
        boundary_leftmost = boundary_leftmost,
        boundary_rightmost = boundary_rightmost,
        boundary_middle = boundary_middle
    )
    seg3 = gen_straight_segment(
        3,
        nlanes,
        roadlength,
        origin = VecSE2(
            origin.x + roadlength + (2*nlanes - 0.5)*lane_width, origin.y - roadlength - 0.5*lane_width, pi/2),
        lane_widths = lane_widths,
        boundary_leftmost = boundary_leftmost,
        boundary_rightmost = boundary_rightmost,
        boundary_middle = boundary_middle
    )
    seg4 = gen_straight_segment(
        4,
        nlanes,
        roadlength,
        origin = VecSE2(
            origin.x + roadlength + 2*nlanes*lane_width, origin.y, 0.0),
        lane_widths = lane_widths,
        boundary_leftmost = boundary_leftmost,
        boundary_rightmost = boundary_rightmost,
        boundary_middle = boundary_middle
    )
    seg5 = gen_straight_segment(
        5,
        nlanes,
        roadlength,
        origin = VecSE2(
            origin.x + roadlength + 2*nlanes*lane_width + roadlength, origin.y + (2*nlanes - 1.0)lane_width, pi),
        lane_widths = lane_widths,
        boundary_leftmost = boundary_leftmost,
        boundary_rightmost = boundary_rightmost,
        boundary_middle = boundary_middle
    )
    seg6 = gen_straight_segment(
        6,
        nlanes,
        roadlength,
        origin = VecSE2(
            origin.x + roadlength + (2*nlanes - 0.5)*lane_width, origin.y + (2*nlanes - 0.5)*lane_width, pi/2),
        lane_widths = lane_widths,
        boundary_leftmost = boundary_leftmost,
        boundary_rightmost = boundary_rightmost,
        boundary_middle = boundary_middle
    )    
    seg7 = gen_straight_segment(
        7,
        nlanes,
        roadlength,
        origin = VecSE2(
            origin.x + roadlength + 0.5*lane_width, origin.y + (2*nlanes - 0.5)*lane_width + roadlength, -pi/2),
        lane_widths = lane_widths,
        boundary_leftmost = boundary_leftmost,
        boundary_rightmost = boundary_rightmost,
        boundary_middle = boundary_middle
    )
    seg8 = gen_straight_segment(
        8,
        nlanes,
        roadlength,
        origin = VecSE2(
            origin.x + roadlength, origin.y + (2*nlanes - 1.0)*lane_width, pi),
        lane_widths = lane_widths,
        boundary_leftmost = boundary_leftmost,
        boundary_rightmost = boundary_rightmost,
        boundary_middle = boundary_middle
...
function add_line!(
    ...
    seg = gen_straight_segment(
        id, nlanes, laneLength,
